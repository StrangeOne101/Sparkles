...
public abstract class FakeParticle extends SpriteTexturedParticle {
    ...
    public static IAnimatedSprite loadTexture(ResourceLocation location) {
        ...
        try (
                Reader reader = new InputStreamReader(Objects.requireNonNull(Sparkle.getResource(fileLoc)),Charsets.UTF_8);
        ) {
            ...
            }).map((resourceLoc) -> Minecraft.getInstance().getAtlasSpriteGetter(resourceLoc).apply(resourceLoc)).collect(Collectors.toList()));

            return texture;
        } catch (IOException e) {
            e.printStackTrace();
        }
        return new FakeParticleTexture(Arrays.asList(Minecraft.getInstance().getAtlasSpriteGetter(MissingTextureSprite.getLocation()).apply(MissingTextureSprite.getLocation())));
    }
